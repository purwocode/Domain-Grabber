...
- Prefix `www.` otomatis dibuang saat scrape (mis. `www.example.com` → `example.com`) supaya domain & subdomain tersimpan rapi dan tidak dobel dengan versi non-www-nya.
- Domain tertentu (mis. `google.com`, `youtube.com`, `facebook.com`, `instagram.com`, `x.com`, `wikipedia.org`, `netflix.com`, `spotify.com`, serta situs/tool AI seperti `chatgpt.com`, `claude.ai`, `perplexity.ai`) dikecualikan secara default — bisa diubah lewat array `exclude`/`excluded` di [popup.js](popup.js) dan [content.js](content.js).
